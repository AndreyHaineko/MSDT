...
## Сдача лабораторной 5 (для ревьюера PR)

**Тестируемый код:** `main.py` — in-memory **сервис пользователей** (`User`, `UserRepository`, `UserService`).

**Тесты:** `tests/test_main.py` — создание, чтение, обновление имени/email, список; моки репозитория; `@pytest.mark.parametrize`.
